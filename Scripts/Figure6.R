################################################################################
##  Figure 6 — Metabolic potential of MAGs by site-preference cluster
##  ---------------------------------------------------------------------------
##  For each metabolic category (Methane, Nitrogen, Sulfur, Carbon fixation and
##  Fermentation) this script counts how many MAGs carry the relevant HMM / KEGG
##  functions, grouped by the genome's "Host Trend Group" — i.e. the treatment
##  in which the MAG is most abundant. The internal group codes (DAM/REST/NAT)
##  are kept as-is in the data; only the displayed labels are set to the study's
##  treatment names:
##      DAM  -> Restored      REST -> Rewetted      NAT  -> Near-natural
##  Panels combine a per-function bar plot with a presence heatmap; all pathway
##  panels are stitched into combined_plot.pdf, with a shared legend.
##
##  Data expected in Data/Figure6/. Outputs (bar plots, combined_plot.pdf,
##  legend_only.png) are written to the working directory.
################################################################################
...
mag_metadata <- read_csv("Data/Figure6/mag_metabolism.csv")
hmm_hits <- read_csv("Data/Figure6/mags_HMMhit.csv")
...
hmm_present <- hmm_present[!hmm_present$Function %in% c("Acetate to acetyl-CoA","Thiosulfate oxidation","Methane oxidation - Partculate methane monooxygenase","Methane oxidation - Soluble methane monoxygenase"),]
...
hmm_selected_clean <- hmm_selected %>%
  ...
  mutate(`Host Trend Group` = recode(`Host Trend Group`,
         ...
         `Host Trend Group` = factor(`Host Trend Group`, levels = c("DAM", "REST", "NAT")),
...
# Load KEGG data
mags_kegg <- read_csv("Data/Figure6/mags_keggmodulehit.csv")
mags_kegg <- mags_kegg[!mags_kegg$Module %in% c("Methane oxidation, methanotroph, methane => formaldehyde","Thiosulfate oxidation by SOX complex, thiosulfate => sulfate",""),]
...
# Add host metadata
taxa_info <- unique(mag_metadata[, c(1, 6)])
...
genecount <- read.csv("Data/Figure6/genecountinbac&arc_kegg.csv")
...
methane_plot <- methane_plot1 + 
  annotation_custom(methane_grob_plot2, xmin = -.1, xmax = 9, ymin = 57, ymax = 60)
...
sulfur_plot <- sulfur_plot + 
  theme(plot.margin = margin(-65, 6.8, 165, 100)) 
...
carbon_plot <- carbon_plot1 + 
  annotation_custom(carbon_grob_plot2, xmin = 0.04, xmax = 7, ymin = 177, ymax = 203) 
